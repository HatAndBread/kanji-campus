const getWidth = () => document.getElementById("can").getBoundingClientRect().width;
...
(function(window, document) {
  ...
  handwriting.Canvas = function(cvs, lineWidth) {
      ...
      this.lastStrokeStyle = window.isDarkMode() ? "white" : "black"
  ...
  handwriting.Canvas.prototype.mouseDown = function(e) {
      // new stroke
      const strokeStyle =  window.isDarkMode() ? "white" : "black";
      if (strokeStyle !== this.lastStrokeStyle) this.erase();
      ...
      this.lastStrokeStyle = strokeStyle;
  };
  ...
  handwriting.Canvas.prototype.undo = function() {
      if (!this.allowUndo || this.step.length <= 0) return;
          ...
          if (this.allowRedo) {
              ...
              this.cxt.clearRect(0, 0, getWidth(), this.height);
          }
      } else {
          if (this.allowRedo) {
              this.redo_step.push(this.step.pop());
  ...
  function loadFromUrl(url, cvs) {
      ...
      imageObj.onload = function() {
          cvs.cxt.clearRect(0, 0, getWidth(), this.height);
  ...
  handwriting.recognize = function(trace, options, callback) {
      ...
      var data = JSON.stringify({
          ...
          "requests": [{
              "writing_guide": {
                  "writing_area_width": getWidth(),
                  "writing_area_height": getWidth()
              },
              "ink": trace,
